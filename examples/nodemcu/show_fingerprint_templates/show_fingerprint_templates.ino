...
void setup()  
{
  ...
  getTemplate(4);   // download the already enrolled template (e.g. #4) into a buffer
  deleteTemplate(4); // delete it at its original location
  sendTemplate(8);// save it from the buffer to another location (e.g. #8); run the fingerprint match example to ensure the template was correctly saved
}
